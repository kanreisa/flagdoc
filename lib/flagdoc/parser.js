...
var regexInstanceProperty = /^(([^#*]+)#([^#]+)) -> ([^)]+)$/;
var regexClassMethod      = /^(([a-zA-Z.]+)\.([^.]+))\(([^)]*)\) -> ([^)]+)$/;
var regexClassProperty    = /^(([a-zA-Z.]+)\.([^.]+)) -> ([^)]+)$/;
...
Parser.prototype.output = function(opt, callback) {
	...
	this.scripts.forEach(function(script) {
		...
		lines.forEach(function(line, i) {
			...
			// instance property
			if (line.match(regexInstanceProperty) !== null) {
				currentKey = line.match(regexInstanceProperty)[1];
				var classKey = line.match(regexInstanceProperty)[2];
				var name     = line.match(regexInstanceProperty)[3];
				var ret      = line.match(regexInstanceProperty)[4];
			...
			// instance property
			if (line.match(regexClassProperty) !== null) {
				currentKey = line.match(regexClassProperty)[1];
				var classKey = line.match(regexClassProperty)[2];
				var name     = line.match(regexClassProperty)[3];
				var ret      = line.match(regexClassProperty)[4];
	...
	var flaglize = function(a) {
		...
		for (var k in dict) {
			...
			str = '<a href="' + getHref(a) + '" title="' + dict[k].key + ' (' + dict[k].type + ')">' + a + '</a>';
	...
	var flaglizeAll = function(str) {
		
		return str.replace(/([a-zA-Z.#]+)/g, function(z, a) {
			if (a.match(/^.+\.$/) === null) {
				return flaglize(a);
			} else {
				return a.replace(/^(.+)(\.)$/, function(y, a, b) {
					return flaglize(a) + (b || '');
				});
			}
		});
	...
	root.forEach(function(a) {
		...
		htmlApiList += '<li class="type-' + a.type + '" data-key="' + a.key + '">' +
		...
		if (a.children) {
			...
			a.children.forEach(function(b) {
				...
				htmlApiList += '<li class="type-' + b.type + '" data-key="' + b.key + '">' +
	...
	templateIndex = templateIndex.replace(/\[\[updatedAt\]\]/g, new Date().toString());
	templateIndex = templateIndex.replace(/\{\{apiList\}\}/g, htmlApiList);
	templatePage  = templatePage.replace(/\[\[product\]\]/g, productName);
	templatePage  = templatePage.replace(/\[\[updatedAt\]\]/g, new Date().toString());
	...
	htmlTop = htmlTop.replace(/\{\{readme\}\}/g, marked(readme) + '<hr><div class="classes"><h3>Classes</h3>' + htmlApiList + '<div style="clear:both;"></div></div>');
	...
	Object.keys(dict).forEach(function(k) {
		...
		htmlPage = htmlPage.replace(/\[\[currentKey\]\]/g, a.key);
		...
		if (a.classKey) {
			htmlPage = htmlPage.replace(/\[\[classKey\]\]/g, a.classKey);
			htmlPage = htmlPage.replace(/\{\{breadcrumbs\}\}/g, '<li><a href="./">Top</a></li><li><a href="' + getHref(a.classKey) + '">' + a.classKey + '</a></li><li>' + getLabel(a) + '</li>');
		} else {
			htmlPage = htmlPage.replace(/\[\[classKey\]\]/g, a.key);
			htmlPage = htmlPage.replace(/\{\{breadcrumbs\}\}/g, '<li><a href="./">Top</a></li><li>' + a.key + '</li>');
		}
		...
		if (a.type !== 'class') {
			...
			if (a.type !== 'instance-property' && a.type !== 'class-property') {
			...
			htmlPage = htmlPage.replace(/\{\{description\}\}/g, flaglizeAll(marked(a.text)));
		} else {
			...
			var iProperties = '<ul class="api-list">';
			a.children.forEach(function(b, i) {
				
				if (b.type === 'instance-property') {
					iProperties += '<li class="type-' + b.type + '">' +
					               '<a href="' + getHref(b.key) + '" title="' + b.key + ' (' + b.type + ')">' +
					               getLabel(b) + '</a></li>';
				}
			});
			...
			var cProperties = '<ul class="api-list">';
			a.children.forEach(function(b, i) {
				
				if (b.type === 'class-property') {
					cProperties += '<li class="type-' + b.type + '">' +
					               '<a href="' + getHref(b.key) + '" title="' + b.key + ' (' + b.type + ')">' +
					               getLabel(b) + '</a></li>';
				}
			});
			...
			var html = '';
			html += flaglizeAll(marked(a.text));
			...
			if (iProperties !== '<ul class="api-list"></ul>') {
				html += '<div class="instance-properties">';
				html += '<h3>Instance properties</h3>';
				html += iProperties;
				html += '<div style="clear:both;"></div></div>';
			}
			...
			if (cProperties !== '<ul class="api-list"></ul>') {
				html += '<div class="class-properties">';
				html += '<h3>Class properties</h3>';
				html += cProperties;
				html += '<div style="clear:both;"></div></div>';
			}
